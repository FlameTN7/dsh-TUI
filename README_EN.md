...
- **Timeline navigation**: a Grok-style turn rail covering **every turn
  ...
  timeline / scrollbar / hidden modes. In scrollbar mode the track is also a
  drag target: an unmodified left drag scrubs the transcript continuously to
  the pointer's track position (a drop-in extension of track clicking), while
  `Shift`/`Alt`/`Ctrl`+drag still selects text.
